...
{      
       if(I_Estop_Get() == SWITCH_OFF_STATE){
           Estop = true;
           iVescCase = 110; 
       //    iVescCase = iVescCase;
        }
    ...
    switch(iVescCase)
    {
        case 0:
           ...
           // lMoveDistance = (lMoveMM/70)*360;   //1.5 inches diameter 4.7124 inches per rev
            ...
            if (I_One_Inch_Sel_Get()) {
                lMoveinch = 1.0;  // length #1
            }
            else if (I_Twenty_Inch_Sel_Get()) {
                lMoveinch = 20.0;  // Length #2
            }
             else if (I_Twenty_Two_Sel_Get()) {
                lMoveinch = 22.0;  //23.5620;  //temp to test stop on one rev 22.0;  // Length #3
            }
             else {
        ...
        case 10:
            PowerupDelay++;
            RestartDelay++;
            if ((PowerupDelay >= 1000)&&(RestartDelay >= 10000)) {
            //PowerupDelay = 0;
                RestartDelay = 0;
                if(VescFlags.bit.ValidMsgs == 1) {
                    ...
                    if (!CAN1_TxFIFOIsFull(0) )
                    {
                        ...
                        {
                            xTxCanData = 0;
                            SetCurrentLimits(feedroller, 100,-100);
                            Delay = 0;  
                            Delay2 = 0;
                            while(!UART1_TransmitComplete());
        ...
        case 12:
          //if(VescFlags.bit.ValidMsgs == 1){
             ...
             if(Delay2 >= 1000) {
               ...
               if (!CAN1_TxFIFOIsFull(0) )
              {
                ...
                if(xTxCanData)
                {
                    //Clear interval bit
                    xTxCanData = 0;
                    SetDuty(feedroller,0.03);  // rotates the taverse int he positive direction in duty cycle to find the encoder positon mark to find commutator angles
                 while(!UART1_TransmitComplete());
                        sprintf((char*)uart1TxBuffer, "iVescCase = %d VescData.RPM = %d \r\n",iVescCase,VescData.RPM);
                        DMAC_ChannelTransfer(DMAC_CHANNEL_0, (const void *)uart1TxBuffer,strlen((const char*)uart1TxBuffer),(const void *)&U1TXREG, 1, 1); 
                     while(!UART1_TransmitComplete());
                }
               }
           ...
           if ((abs(fStartPos - VescData.PidPos) >= 180)&& (VescData.RPM >= 200)) {    
            if(Delay >= 30000)  //100,000  looks like one second
                {
                    Delay = 0;
                    AtDuty = 0;
                    while(!UART1_TransmitComplete());
  ...
  //          iVescCase = 60;
        ...
        case 60:
            ...
            //When Button is pressed start moving the motor or a delay then start
             iVescCase = iVescCase + 2*(((Button1Trigger())||(Start)||(Delay >= 100000)));
